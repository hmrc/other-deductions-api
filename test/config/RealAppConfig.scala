...
trait RealAppConfig {
  ...
  protected lazy val emaEndpoints: Map[String, Boolean] =
    realAppConfig match {
      case impl: OtherDeductionsAppConfigImpl =>
        impl.configuration
  ...
  protected lazy val realAppConfig: OtherDeductionsAppConfig = {
    val conf           = ConfigFactory.load()
    val configuration  = Configuration(conf)
    val servicesConfig = new ServicesConfig(configuration)
    new OtherDeductionsAppConfigImpl(servicesConfig, configuration)
  }
